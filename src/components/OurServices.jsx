...
const OurServices = () => {
  ...
  const cardDescription = "Web development is the art of creating engaging and visually appealing websites";

  return (
    <section className="bg-[#EFFFF9] rounded-[40px] md:rounded-[200px_0px_200px_0px] py-12 md:py-20 px-4 sm:px-10 lg:px-20 xl:px-24">
      <div className="container mx-auto flex flex-col lg:flex-row items-center justify-between gap-10 xl:gap-16">
        ...
        <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start order-2 lg:order-1">
          ...
          <div className="flex flex-col gap-6 sm:mt-20 lg:mt-32">
            <ServiceCard 
              service={services[0]} 
        ...
        <div className="w-full lg:max-w-[450px] xl:max-w-[550px] text-center lg:text-left order-1 lg:order-2">
          <h2 className="text-3xl sm:text-4xl xl:text-5xl font-bold text-[#1a1a1a] mb-6 leading-tight">
            We Offer A Variety Of <br className="hidden xl:block" /> Services Such As
          </h2>
          <p className="text-gray-500 text-sm sm:text-base mb-8 leading-relaxed">
            At our company, we pride ourselves on offering a variety of services to meet the diverse needs of our clients. 
            Whether you're looking for marketing assistance, website design, or IT support, we've got you covered.
          </p>
...
const ServiceCard = ({ service, description, isSpecial }) => {
  return (
    <div className={`group w-full max-w-[300px] sm:w-[240px] md:w-[280px] p-6 md:p-8 rounded-2xl transition-all duration-500 cursor-pointer shadow-sm
    ...
    >
      ...
      <p className={`text-xs md:text-sm leading-relaxed transition-colors ${isSpecial ? 'text-white/90' : 'text-gray-500'} group-hover:text-white`}>
        {description}
      </p>
    </div>
  );
